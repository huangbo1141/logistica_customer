...
public class MultilineTextField: UITextView {
   ...
   private func remove(exlusionPath: UIBezierPath) {
    ...
    public func addBottomLayer(param:CGRect){
        let border = CALayer.init();
        border.borderColor = UIColor.darkGray.cgColor
        let borderWidth = g_txtBorderWidth;
        let frame = CGRect.init(x: 0, y: param.size.height - borderWidth, width: param.size.width, height: borderWidth)
        border.frame = frame
        border.borderWidth = borderWidth
        self.layer .addSublayer(border)
        self.layer.masksToBounds = true
        
//        let paddingView = UIView.init(frame: CGRect.init(x: 0, y: 0, width: 5, height: 20))
//        self.leftView = paddingView
        
        self.bottomLine = border
